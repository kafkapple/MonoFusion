...
def normals_edge(normals: np.ndarray, tol: float, kernel_size: int = 3, mask: np.ndarray = None) -> np.ndarray:
    ...
    if mask is None:
        mid = torch.acos(torch.tensor((normals[..., None, None] * normals_window).sum(axis=-3))).numpy()
        angle_diff = mid.max(axis=(-2, -1))
    else:
        ...
        mid = torch.acos(torch.tensor((normals[..., None, None] * normals_window).sum(axis=-3))).numpy()
        angle_diff = np.where(mask_window, mid, 0).max(axis=(-2, -1))
