    ...
    def __init__(
        ...
        **_,
        ...
        def load_known_cameras(
            ...
            md = json.load(open(f"./data_ego/{seq}/Dy_train_meta.json", 'r'))


            c2ws = []
            for c in range(183, 294):
              h, w = md['hw'][c]
            ...
            fx, fy, cx, cy = k[0, 0],  k[1, 1], k[0, 2], k[1, 2],#recon["intrinsics"]  # (4,)
            K = np.array([[fx * sx, 0, cx * sx], [0, fy * sy, cy * sy], [0, 0, 1]])  # (3, 3)
            Ks = np.tile(K[None, ...], (len(traj_c2w), 1, 1))  # (N, 3, 3)
            kf_tstamps = list(range(183, 294))#recon["tstamps"].astype("int")
            return (
